                                                          ...
                                                          std::vector< FFXIVARR_PACKET_RAW >& packets )
{
  ...
  if( packetHeader.compressionType == Oodle )
  {
    ...
    outBuf.resize( packetHeader.oodleDecompressedLength );

    bool oodleSuccess = false;
    try
    {
      auto _oodle = Network::Oodle();
      oodleSuccess = _oodle.oodleDecode(inBuf, bytesExpected, outBuf, packetHeader.oodleDecompressedLength);
    }
    ...
    bytesExpected = packetHeader.oodleDecompressedLength;
