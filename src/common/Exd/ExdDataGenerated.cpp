...
Sapphire::Data::ContentFinderCondition::ContentFinderCondition(uint32_t row_id, Sapphire::Data::ExdDataGenerated* exdData)
{
  ...
  dutyRecorderAllowed = exdData->getField< bool >(row, 33);
  name = exdData->getField< std::string >(row, 38);
  nameShort = exdData->getField< std::string >(row, 39);
  contentType = exdData->getField< uint8_t >(row, 40);
  transientKey = exdData->getField< uint8_t >(row, 41);
  transient = exdData->getField< uint32_t >(row, 42);
...
Sapphire::Data::Status::Status(uint32_t row_id, Sapphire::Data::ExdDataGenerated* exdData)
{
  ...
  category = exdData->getField< uint8_t >(row, 6);
...
Sapphire::Data::Vase::Vase(uint32_t row_id, Sapphire::Data::ExdDataGenerated* exdData)
{
  auto row = exdData->m_VaseDat.get_row(row_id);
  start = exdData->getField< uint32_t >(row, 0);
  end = exdData->getField< uint32_t >(row, 1);
}
...
Sapphire::Data::VaseFlower::VaseFlower(uint32_t row_id, Sapphire::Data::ExdDataGenerated* exdData)
{
  ...
  flower = exdData->getField< uint16_t >(row, 0);
  isVase = exdData->getField< uint8_t >(row, 1);
  flowerColour = exdData->getField< uint8_t >(row, 2);
  item = exdData->getField< uint32_t >(row, 3);
}
...
bool Sapphire::Data::ExdDataGenerated::init(const std::string& path)
{
  try
  {
    ...
    m_VaseDat = setupDatAccess("Vase", xiv::exd::Language::none);
